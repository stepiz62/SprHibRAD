 ...
 */
public class StatusBar extends FormAccessor {

	@Override
	public int doStartTag() throws JspException {
		String msg =  (String) request().getAttribute("message");
		if (msg == null) {
			NavigatorNode node = getNavigatorNode(0);
			if (node != null) {
				msg = node.message;
				node.message = "";
			}
		...
		try {
			out().append(msg== null || msg.isEmpty() ? "" : ("<table class='status'><tr><td>" + msg + "</td></tr></table>"));
		} catch (IOException e) {
			e.printStackTrace();
		}
